...
uniform float u_shape1Radius;
uniform vec2 u_shape1Center;
...
float mainSDF(vec2 p1, vec2 p2, vec2 p) {
  ...
  float d1 = u_showShape1 == 1 ? roundedRectSDF(
    p1n,
    vec2(0.0),
    u_shape1Width / u_resolution.y,
    u_shape1Height / u_resolution.y,
...
void main() {
  ...
  if (u_bgType <= 0) {
    ...
    vec3 c2 = vec3(0.05, 0.25, 0.35);
    vec3 c3 = vec3(0.45, 0.1, 0.30);
    vec3 c4 = vec3(0.08, 0.15, 0.40);
    float f1 = sin(uv.x * 2.5 + t) * 0.5 + 0.5;
    float f2 = sin(uv.y * 2.0 - t * 0.7 + 1.0) * 0.5 + 0.5;
    float f3 = sin((uv.x + uv.y) * 1.5 + t * 0.5) * 0.5 + 0.5;
    float f4 = cos(uv.x * 1.8 - uv.y * 1.2 + t * 0.8) * 0.5 + 0.5;
    bgColor = mix(c1, c2, f1);
    bgColor = mix(bgColor, c3, f2 * 0.6);
    bgColor = mix(bgColor, c4, f3 * 0.4);
